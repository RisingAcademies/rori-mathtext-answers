...
def build_single_event_nlu_response(
    eval_type, result, confidence=1.0, intents=[], p_learn=0
):
    ...
    {'type': 'comparison', 'data': '25', 'confidence': 1.0, 'p_learn': 0}
    """
    if intents == []:
        return {
            "type": eval_type,
    ...
    intents = [result_obj.copy() for i in range(3)]
